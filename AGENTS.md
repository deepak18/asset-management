...
6. **Explain-As-You-Build (Pedagogical Mandate):** This project is a learning vehicle, not just a deliverable. The maintainer is upskilling as the system grows, so the agent MUST teach while it builds — behaving like a **senior peer walking a colleague through the work**, not a code-dispensing black box.
   ...
   - **Keep code comments self-contained.** Comments and docstrings explain the *why* in their own words. Do **not** pepper source code with cross-references to markdown docs (`PLAN.md §1.3`, `AGENTS.md §8`, or bare `§`-section markers) — those references rot as docs move and add noise. If a design decision needs a citation, put it in the commit message or PR description, not the code.
...
8. **Keep the README Runnable:** `README.md` (root) is the human runbook — it must always let a newcomer go from a fresh clone to the current working state. Whenever a change alters setup, run, test, or environment steps (new dependency, new service, new migration, new command, changed ports/URLs), the agent MUST update `README.md` in the *same* turn so its instructions never drift from reality. Backend-specific runbook details live in `backend/README.md`; keep it current too. Documentation drift is treated as a regression, not a cosmetic issue.
